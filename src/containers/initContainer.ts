...
const createProxyRouter = () => {
  ...
  router.all("*", (_req, res) =>
    res
      .status(503)
      .json({ error: "Service temporarily unavailable - database not ready" })
  );
  return router;
};
...
export const initContainer = (app: express.Express): void => {
  ...
  (async () => {
    ...
    try {
      const container = await retryWithBackoff(
        () => buildContainer(),
        maxAttempts,
        500
      );
      ...
      console.error(
        "DB initialization failed after retries. Exiting process.",
        err
      );
      process.exit(1);
    }

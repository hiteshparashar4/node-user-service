...
import defineUserModel, { User } from "@/models/user";
...
const buildContainer = async (): Promise<AwilixContainer<AppContainer>> => {
  ...
  const UserModel: typeof User = defineUserModel(sequelize);

  const container: AwilixContainer<AppContainer> =
